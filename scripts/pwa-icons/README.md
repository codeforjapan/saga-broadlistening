...
色はパステルオレンジ基調の `sky/500 #ff9440` → `sky/700 #b25200`（`--primary` 系から `--primary-accent` へ）のグラデーションです。`sky/400` 単色は白背景で 1.9:1 しかなく favicon が視認できないため、暗い側に `sky/700`（5.1:1）を置いて輪郭を確保しています。
