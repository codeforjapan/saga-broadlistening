 ...
 * 意見タグ（発言の根拠の種類）の定義。
 *
 * 「専門家の意見だけを見る」を回答者1人に1つ付く立場ラベル（opinions.role_title）
 * だけで実現しようとすると成立しない。自己申告ベースのラベルは専門家として立つ人が
 * ごく少数で、本番実測でも全意見の1%未満しか付かなかった。
 * reasoning_types は論点1件ごとに「その発言が何を根拠にしているか」を持つため、
