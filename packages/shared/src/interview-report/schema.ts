...
export const interviewReportSchema = z
  .object({
    ...
    opinions: z
      ...
      .max(3)
      .describe(
        "ユーザーの具体的な主張（最大3件）。施策を検討する人にとって示唆として有益な順（具体性・建設性・独自性が高い順）に並べ、先頭ほど有益・重要な主張とする。ユーザー本人の発言のみを根拠にし、インタビュアーの発言・言い換え・提示情報をユーザーの意見として記載しない。本人が明言していない要望・賛成・結論への格上げをしない。元の対話ログにないことは記載しない"
      ),
