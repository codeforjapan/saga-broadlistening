...
describe("プリミティブパレット", () => {
  ...
  it("コントラスト比が期待値どおり（対 base-white）", () => {
    // WCAG 2.1 の定義に基づく計算結果。値を動かしたときに「どの用途に使えるか」の
    // 判定（AA 4.5:1 / 3:1）が静かに変わるのを防ぐため、実測値をここに固定する。
    //
    // sky ランプは vanilla のパステルオレンジ基調の実測値で、要求仕様 付録
    // 「コントラスト早見表」の水色基調の値とは一致しない。それ以外は仕様どおりだが、
    // 仕様の表には以下3件の誤りがある（Epic #8 に報告済み）。
