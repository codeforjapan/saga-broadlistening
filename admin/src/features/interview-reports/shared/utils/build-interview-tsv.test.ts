...
describe("buildInterviewTsv", () => {
  ...
  it("1セッション = N行(メッセージ数ぶん)に展開され、セッション情報が冗長コピーされる", () => {
    ...
    expect(cellsRow1[13]).toBe("1");
    expect(cellsRow1[14]).toBe("assistant");
    expect(cellsRow1[15]).toBe("質問1");
    expect(cellsRow2[13]).toBe("2");
    expect(cellsRow2[14]).toBe("user");
    expect(cellsRow2[15]).toBe("回答1");
  });

  it("セッション情報(role_title/score/summary 等)が各行に重複コピーされる", () => {
    const tsv = buildInterviewTsv([
      buildSession({
        interview_report: {
          id: "r1",
    ...
    expect(cells[5]).toBe("教育研究者");
    expect(cells[6]).toBe("published");
    expect(cells[7]).toBe("12");
    expect(cells[8]).toBe("ok");
    expect(cells[9]).toBe("75");
    expect(cells[10]).toBe("true");
    expect(cells[11]).toBe("false");
    expect(cells[12]).toBe("賛成意見");
  });
  ...
  it("改行・タブを半角スペースに置換してTSVの区切りを壊さない", () => {
    ...
    expect(cells[15]).toBe("改行 あり タブも");
  });
  ...
  it("先頭が = + - @ の値は ' を前置して数式評価を防ぐ", () => {
    ...
    const rows = tsv.split("\n");
    expect(rows[1].split("\t")[12]).toBe("'=SUM(A1:A2)");
    expect(rows[1].split("\t")[15]).toBe("'=1+1");
    expect(rows[2].split("\t")[15]).toBe("'@cmd");
    expect(rows[3].split("\t")[15]).toBe("'-1");
    expect(rows[4].split("\t")[15]).toBe("'+SUM");
  });
});
